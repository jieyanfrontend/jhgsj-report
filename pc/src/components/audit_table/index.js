...
let AlertIcon = () => (
  <Icon type="cross-circle-o" className={styles.icon_alert} />
);
let SuccessIcon = () => (
  <Icon type="check-circle-o" className={styles.icon_success} />
);
  ...
  render() {
            ...
            <section className={styles.audit}>
              ...
              <div className={styles.line}>
                <div className={styles.line_item}>
                  ...
                  <span className={styles.message_detail}>
                    {record.org_code || record.uni_soc_cre_code}
                  </span>
                  {record.check &&
                  record.check.org_code === 0 &&
                  record.check.uni_soc_cre_code === 0 ? (
                    <AlertIcon />
                  ) : (
                    <SuccessIcon />
                  )}
              ...
              <div className={styles.line}>
                <div className={styles.line_item}>
                  ...
                  {record.check && record.check.admin === 0 ? (
                    <AlertIcon />
                  ) : (
                    <SuccessIcon />
                  )}
              ...
              <div className={styles.line}>
                <div className={styles.line_item}>
                  <span className={styles.message_title}>上传时间:</span>
                  <span className={styles.message_detail}>
                    {record.create_time}
                  </span>
                  {record.check && record.check.create_time === 0 ? (
                    <AlertIcon />
                  ) : (
                    <SuccessIcon />
                  )}
              ...
              <div className={styles.line}>
                <div className={styles.line_item}>
                  ...
                  {record.check && record.check.address === 0 ? (
                    <AlertIcon />
                  ) : (
                    <SuccessIcon />
                  )}
    ...
    if (state === 0) {
      columns.push({
        title: "操作",
        render: (text, record) => (
          <div className={styles.audit_btn_group}>
            <Button type="primary" onClick={() => pass(record.id)}>
              审核通过
            </Button>
            <Button onClick={() => this.handleMsg(record.id)}>
              审核不通过
            </Button>
          </div>
        )
    ...
    return (
      <div>
        <RemarkModal
          visible={isRemarking}
          setVisible={bool => this.setVisible({ isRemarking: bool })}
          ensure={msg => fail({ id, msg }, this.setVisible)}
        />
        <PreviewModal
          url={url}
          visible={isPreviewing}
          setVisible={bool => this.setVisible({ isPreviewing: bool })}
        />
        <Table
          loading={loading}
          columns={columns}
